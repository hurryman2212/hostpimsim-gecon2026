...
- Nested invocation of the same DPU handler for the same region and offset will generate undefined behavior.
...
- **Root + instance model (vsysfs)** — create a root bound to a `pim_vdev_root_t` with `pim_vsysfs_root_create(vdev_root, class_name, kobject_name, instance_name_glob)`, then create concrete entries with `pim_vsysfs_create(root, vdev, instance_name)`
- **Virtual sysfs attributes** — `pim_vsysfs_t` objects serve key-value attributes from concrete instance directories
- **Derived vsysfs attrs are auto-created** — `pim_vsysfs_create()` populates `dev`, `subsystem`, and `uevent` from vdev/root metadata (unless already provided by root defaults). Limitation: `power` is intentionally not auto-populated yet, so runtime-PM style entries under `power/` (`autosuspend_delay_ms`, `control`, `runtime_active_time`, `runtime_status`, `runtime_suspended_time`) are not synthesized.
...
- Attribute permission is configured per key via `pim_vsysfs_write_attr(..., access)` using `PIM_ACCESS_MODE_NONE`, `PIM_ACCESS_MODE_RO`, `PIM_ACCESS_MODE_RW`, or `PIM_ACCESS_MODE_WO`.
...
pim_vmodule_register_driver(vmodule, "platform", "dpu_region_mem");
...
// pim_vsysfs_root_write_default_attr(vsysfs_dax_root, "size", "8589934592\n", PIM_ACCESS_MODE_RO);
// pim_vsysfs_root_write_default_attr(vsysfs_dax_root, "numa", "0\n", PIM_ACCESS_MODE_RO);

// 4. Create concrete vdev and vsysfs instances
// /dev/dpu_rank0
pim_vdev_t *vdev_rank0 = pim_vdev_create(vdev_rank_root,
    "dpu_rank0", 240, 0); // Return error if the name is already being used.
pim_vsysfs_t *vsysfs_rank0 = pim_vsysfs_create(vsysfs_rank_root,
    vdev_rank0, "dpu_region_mem.0");
// /dev/dax0.0
// pim_vdev_t *vdev_dax0_0 = pim_vdev_create(vdev_dax_root, "dax0.0", 240, 1);
...
// pim_vsysfs_write_attr(vsysfs_dax0_0, "size", "8589934592\n", PIM_ACCESS_MODE_RO);
// pim_vsysfs_write_attr(vsysfs_dax0_0, "numa", "0\n", PIM_ACCESS_MODE_RO);
...
| `pim_device_user_lock(dev)`             | Acquire user-level device lock                                                                   |
| `pim_device_user_unlock(dev)`           | Release user-level device lock                                                                   |
...
> - `pim_device_user_lock()` / `pim_device_user_unlock()` provide a user-level device mutex for caller-controlled critical sections
> `pim_device_barrier(dev)` blocks until all submitted pool tasks on the device finish; It will return `PIM_ERR_WOULD_DEADLOCK` when called from a pool-worker context.
...
| `pim_region_get_offset(region)`              | Get region offset in backing shared memory address range |
| `pim_region_get_device(region)`              | Get parent device                                        |
| `pim_region_user_lock(region)`               | Acquire user-level region lock                           |
| `pim_region_user_unlock(region)`             | Release user-level region lock                           |
...
| `pim_region_barrier(region)`                 | Wait for pool tasks submitted for that region            |
...
Behavior notes:
- `pim_device_user_lock()` / `pim_device_user_unlock()` and `pim_region_user_lock()` / `pim_region_user_unlock()` are user synchronization helpers for coordinating caller-owned critical sections.
- These user lock APIs do NOT gate internal runtime operations (monitor/pool/SIGSEGV state transitions, topology mutation checks, or handler dispatch).
- The user lock APIs are non-recursive (`std::mutex` semantics); locking the same object twice from the same thread will deadlock.
- `pim_region_barrier(region)` blocks until submitted tasks associated with that region finish.

### DPU Handlers

`pim_dpu_handler_t` is the callback type used by `pim_register_dpu_handler()`:
...
- `dev`: Device that owns the triggering region.
- `region`: `PIM_REGION_CTRL_MMIO` region where the handler was matched.
- `offset`: Byte offset (within `region`) of the triggering register.
- `value`: Register payload passed to the callback.
- `user_data`: Opaque pointer supplied at registration time; ownership stays with caller.

Execution model:
- If the pool is running, callbacks are usually executed by pool workers.
- If the pool is not running, callbacks execute directly in the detecting context (monitor thread, SIGSEGV callback thread, or caller thread for `pim_invoke_dpu_handler()`).
- Callbacks may run concurrently; handler code and `user_data` access should be thread-safe.
- In C++ builds, exceptions thrown by the callback are caught and ignored at the library boundary to preserve `noexcept` public API behavior.

Lifetime note:
- Keep `user_data` and any referenced state valid until the callback can no longer run (for async paths, use `pim_region_barrier(region)` or `pim_device_barrier(dev)` before teardown).

| Function                                                                         | Description                                       |
| -------------------------------------------------------------------------------- | ------------------------------------------------- |
| `pim_register_dpu_handler(dev, region, offset, depth, size, handler, user_data)` | Register handler                                  |
| `pim_unregister_dpu_handler(dev, region, offset, depth)`                         | Remove handler at offset/depth                    |
| `pim_invoke_dpu_handler(region, offset, depth, value)`                           | Directly invoke handler without writing to memory |
...
Handler table mutation APIs (`pim_register_dpu_handler` / `pim_unregister_dpu_handler`) are rejected with `PIM_ERR_BUSY` while monitor threads are active, while SIGSEGV mode is active, during SIGSEGV transition windows, or while device teardown is in progress.

`pim_invoke_dpu_handler()` looks up the handler registered at `(offset, depth)` and calls it with the provided `value`, without performing any memory write. The device is inferred from `region`. This is the recommended dispatch method when `pim_write(...)` cannot be used — for example, when the hardware uses interleaved or encoded memory writes that are decoded externally (e.g., in simulator-side decode loops or vdev ioctl handlers). If a thread pool is running, the handler is submitted to the pool; otherwise it runs synchronously.
...
| `pim_write_sync(region, offset, size, value)` | Trigger + wait until submitted pool tasks for that region are finished                   |
...
| Function                          | Description                                                                    |
| --------------------------------- | ------------------------------------------------------------------------------ |
| `pim_pool_start(dev, n)`          | Start thread pool (`n=0` uses hardware CPU thread count)                       |
...
| `pim_get_hardware_concurrency()`  | Get host hardware thread concurrency hint                                      |

Call `pim_pool_start(dev, n)` before starting detection. Pass `n=0` to use `std::thread::hardware_concurrency()` (fallback to 1 if unavailable). Use `pim_set_max_active_dpus()` to resize dynamically while running.
...
| `pim_vdev_create(root, name, major, minor)` | Create concrete vdev instance with device numbers (e.g., `"dpu_rank0", 240, 0`). Pass `-1` for `major` and/or `minor` to auto-assign the next available value. |
...
| `pim_vdev_get_major(vdev)`                  | Get major number configured on the vdev (`-1` on invalid input).                                                                                               |
| `pim_vdev_get_minor(vdev)`                  | Get minor number configured on the vdev (`-1` on invalid input).                                                                                               |
...
| Function                                                                          | Description                                                                                                                                                                                    |
| --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pim_vsysfs_root_create(vdev_root, class_name, kobject_name, instance_name_glob)` | Create a vsysfs root bound to a vdev root. Class side is `/sys/class/<class_name>/<vdev_name>`; devices side is `/sys/devices/<kobject_name>/<instance_name>/<vdev_name>` (when names differ). |
...
| `pim_vsysfs_root_write_default_attr(root, name, value, access)`                   | Set default attribute and access mode for instances under this root.                                                                                                                           |
...
| `pim_vsysfs_write_attr(vsysfs, name, value, access)`                              | Set/update instance attribute and access mode.                                                                                                                                                 |
...
| `pim_vmodule_register_driver(vmodule, bus_name, driver_name)` | Register a bus driver and create `/sys/bus/<bus_name>/drivers/<driver_name>`. |
