 ...
 * @param depth     Handler depth level (0 = root/default)
...
/**
 * @brief Acquire user-level device lock
 *
 * @param dev       Device handle
 * @return          PIM_SUCCESS or PIM_ERR_INVALID_ARG
 */
pim_error_t pim_device_user_lock(pim_device_t *dev) PIM_NOEXCEPT;

/**
 * @brief Release user-level device lock
 *
 * @param dev       Device handle
 * @return          PIM_SUCCESS or PIM_ERR_INVALID_ARG
 */
pim_error_t pim_device_user_unlock(pim_device_t *dev) PIM_NOEXCEPT;
...
/**
 * @brief Acquire user-level region lock
 *
 * @param region    Region handle
 * @return          PIM_SUCCESS or PIM_ERR_INVALID_ARG
 */
pim_error_t pim_region_user_lock(pim_region_t *region) PIM_NOEXCEPT;

/**
 * @brief Release user-level region lock
 *
 * @param region    Region handle
 * @return          PIM_SUCCESS or PIM_ERR_INVALID_ARG
 */
pim_error_t pim_region_user_unlock(pim_region_t *region) PIM_NOEXCEPT;
...
pim_error_t pim_invoke_dpu_handler(pim_region_t *region, size_t offset,
                                   uint32_t depth, uint64_t value) PIM_NOEXCEPT;
...
/**
 * @brief Wait until submitted pool tasks for this region are finished
 *
 * Uses per-region counters (`futex_cnt_commited` / `futex_cnt_finished`) and
 * futex wait/wake on `futex_cnt_finished`.
 *
 * @param region    Region handle
 * @return          PIM_SUCCESS, PIM_ERR_INVALID_ARG, or PIM_ERR_WOULD_DEADLOCK
 */
pim_error_t pim_region_barrier(pim_region_t *region) PIM_NOEXCEPT;
...
/**
 * @brief Write a typed register value and wait for region pool quiescence
 *
 * Equivalent to `pim_write(...)` followed by
 * `pim_region_barrier(region)`.
 ...
 *                    If 0, uses std::thread::hardware_concurrency().
 *                    If hardware concurrency is unavailable, falls back to 1.
 ...
 * - Increasing appends additional worker threads in-place
 * - Decreasing rebuilds the pool to the requested size
 *
...
/**
 * @brief Get host hardware concurrency hint
 *
 * Returns std::thread::hardware_concurrency() when available.
 * If unavailable (returns 0), this API returns 1.
 *
 * @return                  Hardware concurrency value (always >= 1)
 */
uint32_t pim_get_hardware_concurrency(void) PIM_NOEXCEPT;
 ...
 * @param major     Device major number used for stat metadata.
 *                  Pass -1 to auto-assign.
 * @param minor     Device minor number used for stat metadata.
 *                  Pass -1 to auto-assign.
...
/**
 * @brief Get virtual device major number
 *
 * @param vdev      Virtual device instance
 * @return          Major number, or -1 on invalid input
 */
int pim_vdev_get_major(pim_vdev_t *vdev) PIM_NOEXCEPT;

/**
 * @brief Get virtual device minor number
 *
 * @param vdev      Virtual device instance
 * @return          Minor number, or -1 on invalid input
 */
int pim_vdev_get_minor(pim_vdev_t *vdev) PIM_NOEXCEPT;
 ...
 *                            (e.g., "dpu_rank")
 * @param kobject_name        Kobject directory name under /sys/devices
 *                            (e.g., "platform")
 * @param instance_name_glob  Instance glob under /sys/devices/<kobject_name>
 *                            (e.g., "dpu_region_mem.*")
 * @return                    Root handle, or NULL on failure
 */
pim_vsysfs_root_t *
pim_vsysfs_root_create(pim_vdev_root_t *vdev_root, const char *class_name,
                       const char *kobject_name,
 ...
 */
pim_error_t
pim_vsysfs_root_write_default_attr(pim_vsysfs_root_t *root, const char *name,
                                   const char *value,
                                   pim_access_mode_t access) PIM_NOEXCEPT;
...
/**
 * @brief Write (or update) an attribute in a virtual sysfs directory
 *
 ...
 * @return          PIM_SUCCESS, PIM_ERR_INVALID_ARG, or PIM_ERR_NO_MEMORY
 */
pim_error_t pim_vsysfs_write_attr(pim_vsysfs_t *vsysfs, const char *name,
                                  const char *value,
                                  pim_access_mode_t access) PIM_NOEXCEPT;
...
/**
 * @brief Register a virtual bus driver for this module
 *
 * Creates virtual bus driver directories:
 *   /sys/bus/<bus_name>/drivers/<driver_name>
 *
 * Duplicate registrations on the same module are ignored and treated as
 * success.
 *
 * @param vmodule      Virtual module instance
 * @param bus_name     Bus name (e.g., "platform")
 * @param driver_name  Driver name (e.g., "dpu_region_mem")
 * @return             PIM_SUCCESS, PIM_ERR_INVALID_ARG, or PIM_ERR_NO_MEMORY
 */
pim_error_t pim_vmodule_register_driver(pim_vmodule_t *vmodule,
                                        const char *bus_name,
                                        const char *driver_name) PIM_NOEXCEPT;
