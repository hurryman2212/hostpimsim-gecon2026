...
int vdev_open_cb(pim_vdev_t *vdev, const char *path, int flags) {
  ...
  if (!vdev_ud || vdev_ud->owner_pid != -1) {
    errno_save = EINVAL;
    goto out;
  }
  type = vdev_ud->type;
...
long rank_ioctl_cb(pim_vdev_t *vdev, int fd, unsigned long cmd,
  ...
  upmem_device_user_data *device_ud =
      static_cast<upmem_device_user_data *>(pim_device_get_user_data(device));
  if (!device_ud || !device_ud->rank) {
    errno = ENODEV;
    pim_vdev_unlock(vdev);
    return -1;
  }

  upmem_pim_rank *rank = device_ud->rank;
  switch (cmd) {
