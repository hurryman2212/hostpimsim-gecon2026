...
struct pim_region {
  ...
  std::mutex mutex;      /**< Handler-table thread safety */
  std::mutex user_mutex; /**< User-level region lock/unlock state */
  std::atomic<uint32_t> futex_cnt_commited{
      0}; /**< Region-submitted pool tasks */
  std::atomic<uint32_t> futex_cnt_finished{
      0}; /**< Region-finished pool tasks */
};

/**
 * @brief Thread-pool queue entry.
 */
struct pool_task_entry {
  pim_region_t *region{nullptr};      /**< Region associated with this task */
  size_t offset{0};                   /**< Register offset within region */
  uint32_t depth{0};                  /**< Nested handler depth */
  uint64_t value{0};                  /**< Handler input value */
  pim_dpu_handler_t handler{nullptr}; /**< Handler callback */
  void *user_data{nullptr};           /**< Handler user data */
};
...
struct pim_device {
  ...
  std::mutex user_mutex; /**< User-level device lock/unlock state */
  ...
  std::queue<pool_task_entry> task_queue;   /**< Pending handler tasks */
 ...
 */
static inline bool pool_submit(pim_device_t *dev, pim_region_t *region,
  ...
  try {
    ...
    dev->task_queue.push(std::move(entry));
  } catch (...) {
    return false;
  }
  ...
  if (region) {
    (void)region->futex_cnt_commited.fetch_add(1u, std::memory_order_acq_rel);
  }
  return true;
}
