 ...
 */
static inline void invoke_dpu_handler_noexcept(pim_device_t *dev,
                                               pim_region_t *region,
  ...
  try {
    handler(dev, region, offset, value, user_data);
  } catch (...) {
 ...
 */
static inline void dispatch_resolved_dpu_handler(pim_device_t *dev,
                                                 pim_region_t *region,
                                                 size_t offset, uint64_t value,
                                                 pim_dpu_handler_t handler,
                                                 void *user_data) {
  ...
  if (dev->pool_running.load(std::memory_order_relaxed)) {
    try {
      if (!pool_submit(dev, [dev, region, offset, value, handler, user_data]() {
            invoke_dpu_handler_noexcept(dev, region, offset, value, handler,
                                        user_data);
          })) {
        invoke_dpu_handler_noexcept(dev, region, offset, value, handler,
                                    user_data);
    ...
    invoke_dpu_handler_noexcept(dev, region, offset, value, handler, user_data);
  }
}
...
static void write_and_invoke_handler(pim_region_t *region, size_t offset,
  ...
  *target = value;
  ...
  // In SIGSEGV mode, signal callback thread dispatches the handler.
  if (!sigsegv_active && handler && dev) {
    dispatch_resolved_dpu_handler(
        dev, region, offset, static_cast<uint64_t>(value), handler, user_data);
  }
}
...
void pim_device_set_user_data(pim_device_t *dev, void *user_data) noexcept {
  if (!dev) {
    return;
  }
  std::lock_guard<std::mutex> lock(dev->mutex);
...
void *pim_device_get_user_data(pim_device_t *dev) noexcept {
  if (!dev) {
    return nullptr;
  }
  std::lock_guard<std::mutex> lock(dev->mutex);
...
pim_error_t pim_invoke_dpu_handler_internal(pim_device_t *dev,
  ...
  dispatch_resolved_dpu_handler(dev, region, offset, value, handler, user_data);
...
pim_error_t pim_device_barrier(pim_device_t *dev) noexcept {
  ...
  for (;;) {
    const uint32_t copy_futex_cnt_finished =
        dev->futex_cnt_finished.load(std::memory_order_acquire);
    const uint32_t futex_cnt_commited =
        dev->futex_cnt_commited.load(std::memory_order_acquire);

    if (copy_futex_cnt_finished == futex_cnt_commited) {
      return PIM_SUCCESS;
    }

    (void)futex(reinterpret_cast<int *>(&dev->futex_cnt_finished),
                FUTEX_WAIT_PRIVATE, static_cast<int>(copy_futex_cnt_finished),
                nullptr, nullptr, 0);
  }
}
