...
static inline bool vsysfs_ensure_parent_device_node(
  ...
  try {
    subsystem_value = std::string("/sys/bus/") + root_shared->kobject_name;
    class_dir = subsystem_value + "/devices/" + instance_name;
    device_value = parent_devices_dir;
    uevent_value = std::string("DEVPATH=/devices/") +
                   root_shared->kobject_name + "/" + instance_name +
                   "\nSUBSYSTEM=" + root_shared->kobject_name +
...
pim_vsysfs_root_create(pim_vdev_root_t *vdev_root, const char *class_name,
  ...
  if (!vdev_root || !class_name || !kobject_name || !instance_name_glob) {
    return nullptr;
  }
  ...
  return vsysfs_try_or_nullptr<pim_vsysfs_root_t>([&]() -> pim_vsysfs_root_t * {
    ...
    root->kobject_name =
        vsysfs_normalize_devices_component(std::string(kobject_name));
    ...
    if (!vsysfs_root_component_valid(root->class_name) ||
        !vsysfs_root_component_valid(root->kobject_name) ||
        root->instance_name_glob.empty() || vdev_name_glob.empty()) {
      return nullptr;
    }

    root->class_glob = root->class_name + "/" + vdev_name_glob;
    root->devices_glob = root->kobject_name + "/" + root->instance_name_glob;
...
pim_vsysfs_t *pim_vsysfs_create(pim_vsysfs_root_t *root, pim_vdev_t *vdev,
  ...
  return vsysfs_try_or_nullptr<pim_vsysfs_t>([&]() -> pim_vsysfs_t * {
    std::string cls;
    unsigned int vdev_major = 0;
    unsigned int vdev_minor = 0;
    {
      std::lock_guard<std::mutex> vdev_lock(vdev_shared->mutex);
      cls = vdev_shared->name;
      vdev_major = vdev_shared->major;
      vdev_minor = vdev_shared->minor;
    }
    ...
    const std::string devices_parent_dir =
        std::string("/sys/devices/") + root_shared->kobject_name + "/" + inst;
    ...
    const pim_error_t root_link_ret = vsysfs_try_or_pim_no_memory([&]() {
      ...
      // Auto-populate derived attrs unless explicitly provided by defaults.
      auto ensure_attr = [&](const char *key, std::string value,
                             pim_access_mode_t access) {
        if (!key) {
          return;
        }
        const std::string attr_name(key);
        if (vsysfs->attrs.find(attr_name) != vsysfs->attrs.end()) {
          return;
        }
        vsysfs->attrs.emplace(attr_name, std::move(value));
        vsysfs->attr_access[attr_name] = access;
      };

      ensure_attr("dev",
                  std::to_string(vdev_major) + ":" + std::to_string(vdev_minor),
                  PIM_ACCESS_MODE_RO);
      ensure_attr("subsystem",
                  std::string("/sys/class/") + root_shared->class_name,
                  PIM_ACCESS_MODE_RO);

      std::string devpath =
          vfile_strip_known_prefix(vsysfs->devices_dir, "/sys");
      if (devpath.empty() || devpath.front() != '/') {
        devpath = "/" + devpath;
      }
      ensure_attr("uevent",
                  std::string("DEVPATH=") + devpath + "\nSUBSYSTEM=" +
                      root_shared->class_name + "\nDEVNAME=" + cls +
                      "\nMAJOR=" + std::to_string(vdev_major) +
                      "\nMINOR=" + std::to_string(vdev_minor),
                  PIM_ACCESS_MODE_RO);
