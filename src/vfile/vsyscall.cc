...
static inline bool vsyscall_apply_stat_kind(vsyscall_stat_kind kind,
                                            size_t payload_size,
                                            pim_access_mode_t attr_access,
                                            struct stat *stbuf,
                                            int *stat_errno) noexcept {
  if (!stbuf || !stat_errno) {
    return false;
  ...
  case vsyscall_stat_kind::CHR_DEVICE:
    ...
    stbuf->st_rdev =
        makedev(kVsyscallVirtualChrRdevMajor,
                kVsyscallVirtualChrRdevMinor); /* /dev/null-like metadata */
    *stat_errno = 0;
    return true;
...
vsyscall_build_vdev_stat(const std::shared_ptr<vfile_fd_entry> &entry,
  ...
  return vsyscall_apply_stat_kind(kind, payload_size, attr_access, stbuf,
                                  stat_errno);
}
...
#endif
  const int flags_err =
      vsyscall_validate_known_flags(flags, supported_flags);
  if (flags_err != 0) {
    return flags_err;
  }
...
static long dispatch_readlinkat(int dfd, const char *pathname, char *buf,
  ...
  if (resolved_path.abs_path == "/sys/class/dpu_rank/subsystem") {
    return vsyscall_return_readlink_target("/sys/subsystem/dpu_rank", buf,
                                           bufsiz, forward);
  }
  if (resolved_path.abs_path == "/sys/class/dpu_dax/subsystem") {
    return vsyscall_return_readlink_target("/sys/subsystem/dpu_dax", buf,
                                           bufsiz, forward);
  }
