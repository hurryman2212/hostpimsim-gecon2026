...
vmodule_normalize_component_name(std::string name) noexcept {
...
vmodule_normalize_instance_name(std::string name) noexcept {
  ...
  return vmodule_normalize_component_name(std::move(name));
}

static inline std::string
vmodule_normalize_bus_name(std::string name) noexcept {
  name = vfile_strip_known_prefix(std::move(name), "/sys/bus/");
  return vmodule_normalize_component_name(std::move(name));
}

static inline std::string
vmodule_normalize_driver_name(std::string name) noexcept {
  const std::string drivers_prefix("/drivers/");
  const size_t drivers_pos = name.rfind(drivers_prefix);
  if (drivers_pos != std::string::npos) {
    name.erase(0, drivers_pos + drivers_prefix.size());
  }
  return vmodule_normalize_component_name(std::move(name));
}
...
pim_error_t pim_vmodule_register_driver(pim_vmodule_t *vmodule,
  ...
  if (!vmodule || !bus_name || !driver_name) {
    return PIM_ERR_INVALID_ARG;
  }

  std::string normalized_bus(bus_name);
  std::string normalized_driver(driver_name);
  normalized_bus = vmodule_normalize_bus_name(std::move(normalized_bus));
  normalized_driver =
      vmodule_normalize_driver_name(std::move(normalized_driver));
  if (!vmodule_valid_instance_name(normalized_bus) ||
      !vmodule_valid_instance_name(normalized_driver)) {
    return PIM_ERR_INVALID_ARG;
  }

  const std::pair<std::string, std::string> registration(
      std::move(normalized_bus), std::move(normalized_driver));
  bool inserted = false;
  const pim_error_t set_ret = vmodule_try_or_pim_no_memory([&]() {
    std::lock_guard<std::mutex> lock(vmodule->mutex);
    inserted = vmodule->registered_drivers.emplace(registration).second;
  });
  ...
  const pim_error_t set_ret = vmodule_try_or_pim_no_memory([&]() {
    ...
    if (had_attr) {
      ...
      const pim_access_mode_t old_access =
          (access_it != vmodule->attr_access.end()) ? access_it->second
                                                    : PIM_ACCESS_MODE_RW;
